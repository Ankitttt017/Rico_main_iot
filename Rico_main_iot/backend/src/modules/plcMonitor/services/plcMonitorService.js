...
async function getReadingHistory({ ip, limit = 200, from, to, page, pageSize, shotNumber, shift, shotResult } = {}) {
  ...
  const appendProductionFilters = (filters, values) => {
    ...
    if (shift && shift !== "all") {
      const hourExpr = "COALESCE(TRY_CONVERT(INT, shot_hour), DATEPART(hour, shot_datetime), DATEPART(hour, recorded_at))";
      const minuteExpr = "COALESCE(TRY_CONVERT(INT, shot_minute), DATEPART(minute, shot_datetime), DATEPART(minute, recorded_at))";
...
function startPlcMonitor(io) {
  ...
  const readAll = async (
    ...
    const payload = {
      ...
      rawReadings: withoutStoppageEventFields(readings),
